...
RUN_ON_CONDOR_SCRIPT = 'runOnCondor2.sh' #runs an executable on condor, (loads root, etc)
...
def writeSubmitScript(scriptfile, runFolder, runOnCondor, EXECUTABLE, inputfilePath,outputIdentifier, start, end):
    ...
    scriptfile.write("arguments              ="+EXECUTABLE+" "+inputfile+" "+outputfile+" "+ str(start)+" " +str(end)+ "\n")
    scriptfile.write("output                 ="+SAMPLE_DIR+outputIdentifier+"-"+str(start)+".out\n")
    scriptfile.write("error                  ="+SAMPLE_DIR+outputIdentifier+"-"+str(start)+".err\n")
    scriptfile.write("log                    ="+SAMPLE_DIR+outputIdentifier+"-"+str(start)+".log\n")
...
inputfilePath = '/uscms/home/wnash/eos/Charmonium/charmonium2016F/161031_1/0000/CSCDigiTree_1.root'
f = r.TFile(inputfilePath)
t = f.CSCDigiTree
entries = t.GetEntries()
jobsize = 10000
divisions = entries / jobsize
for num in range(0, divisions):
    ...
    thisOutputIdentifier = OUTPUT_IDENTIFIER+'-'+str(num)
    start = num*jobsize
    end = (num+1)*jobsize
    ...
    writeSubmitScript(scriptfile, runFolder, RUN_ON_CONDOR_SCRIPT, EXECUTABLE, inputfilePath,thisOutputIdentifier, start, end)
    scriptfile.close()

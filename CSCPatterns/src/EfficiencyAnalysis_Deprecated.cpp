...
int EfficiencyAnalysis_Deprecated() {
	TFile* f = TFile::Open("~/CSCPatterns/data/CSCDigiTree161031.root");
	if(!f)
	{
	...
	//
	/*
	TH2F* patternComparison = new TH2F("h1", "h1",setOneSize+1, -0.5,setOneSize+0.5,
			setTwoSize, -0.5,setTwoSize+0.5);
	 */
	int maxPt = 60;
	...
	testSet->push_back(id3);
	testSet->push_back(id2);
	...
	if(THREE_WIDE){
		matchPlotTitle = "3-Wide Pattern Set";
		setOne = firstNewSet;
	} else {
		matchPlotTitle = "Current Pattern Set";
		setOne = BaseSet;
	}
	//vector<ChargePattern>* setTwo = BaseSet;


	for(unsigned int i =0; i < setOne->size(); i++){
		printPattern(setOne->at(i));
	}
	...
	if(true) {
		vector<ChargePattern>* thisPatternSet = fullSet.m_patterns;
			for(unsigned int i = 0; i < thisPatternSet->size(); i++){
				...
				fullSet.m_matchIds.push_back(thisHistIds);

			}
			TH1F* missedID = new TH1F(string("missedID"+to_string(1234)).c_str(), "missedID", 2*maxPt, 0,maxPt);
			missedID->SetFillColor(kBlack);
			missedID->SetLineColor(kBlack);
			fullSet.m_plots.push_back(missedID);

			TH1F* thisDenom = new TH1F(string("div"+to_string(1234)).c_str(),string("div"+to_string(1234)).c_str(),2*maxPt, 0,maxPt);
			fullSet.m_denominator = thisDenom;
	}
		  ...
		  // IGNORE SEGMENTS AT THE EDGES OF THE CHAMBERS
			...
			for(unsigned int icomp = 0; icomp < compId->size(); icomp++){
				...
				for(unsigned int icompstr = 0; icompstr < compStr->at(icomp).size(); icompstr++){
					...
					//int halfStripVal = 2*(compStrip-1)+compHStrip;
					...
					if(halfStripVal >= (int)N_MAX_HALF_STRIPS || halfStripVal < 0) {
			...
			//find out where the hits are for this segment
			for(unsigned int thisRh = 0; thisRh < rhId->size(); thisRh++)
			{
				...
				if(iRhStrip >= (int)N_MAX_HALF_STRIPS || iRhStrip < 0){
			...
			if(!thisSetMatch.size()) {
				...
				if(maxLayerMatchCount <  N_LAYER_REQUIREMENT && maxLayerMatchCount != nRh){
					idHist->Fill(0);
				}else {
					idHist->Fill(maxLayerId/2);
				}

				if(groupIndex == TESTING_GROUP_INDEX){
					denominator->Fill(Pt);
					if(maxLayerMatchCount == nRh){
						matched->Fill(Pt);
					} else if(maxLayerMatchCount < N_LAYER_REQUIREMENT){
						missed->Fill(Pt);
					} else if(maxLayerMatchCount == nRh - 1){
						missingOne->Fill(Pt);

					} else if(maxLayerMatchCount == nRh - 2){
						missingTwo->Fill(Pt);
					} else if(maxLayerMatchCount == nRh - 3){
				...
				//also require at least 4 layers?
				if(nRh >= N_LAYER_REQUIREMENT){
	...
	THStack* stack = new THStack("hs", string("Pattern Matched Distribution for " + matchPlotTitle + "(" +
			matchGroups[TESTING_GROUP_INDEX].m_name + ")").c_str());
	stack->Add(missed);
	...
	legend->AddEntry(missed,"Matched < 3 RecHits");
	legend->Draw();
	...
	TCanvas* c2 = new TCanvas("c2","comp2",1200,900);
	c2->cd();
	c2->SetLogy();


	idHist->Scale(1./idHist->Integral());
	idHist->GetXaxis()->SetBinLabel(1,"Missed");
	idHist->GetXaxis()->SetBinLabel(2,"ID2/3");
	idHist->GetXaxis()->SetBinLabel(3,"ID4/5");
	idHist->GetXaxis()->SetBinLabel(4,"ID6/7");
	idHist->GetXaxis()->SetBinLabel(5,"ID8/9");
	idHist->GetXaxis()->SetBinLabel(6,"IDA");
	idHist->Draw("hist");
	...
	for(unsigned int igroup = 0; igroup < nMatchGroups; igroup++){
		...
		vector<TH1F*>* idMatchPlots = &matchGroups[igroup].m_plots; //just so i don have to rewrite all this

		std::reverse(idMatchPlots->begin(), idMatchPlots->end());

		THStack* stack2 = new THStack(string("hs2"+to_string(igroup)).c_str(),
				string("Matched (#geq " + to_string(N_LAYER_REQUIREMENT) +
				" rH) for " + matchPlotTitle + " ("+
		...
		stack2->GetYaxis()->SetTitle("Efficiency / 0.5 GeV");
		stack2->GetYaxis()->CenterTitle();



		auto legend2 = new TLegend(0.6,0.5,0.85,0.85);
		for(unsigned int ihist = 0; ihist < idMatchPlots->size(); ihist++){
	...
	c4->SetLogy();
	c4->cd();
	vector<TH1F*>* idMatchPlots2 = &fullSet.m_plots;


	std::reverse(idMatchPlots2->begin(), idMatchPlots2->end());
	THStack* stack3 = new THStack("hs3_69",
			string("Matched (#geq " + to_string(N_LAYER_REQUIREMENT) +
			" rH) for " + matchPlotTitle + " ("+
	...
	stack3->Draw();
	stack3->GetXaxis()->SetTitle("p_{T} [GeV]");
	stack3->GetXaxis()->CenterTitle();
	stack3->GetYaxis()->SetTitle("Efficiency / 0.5 GeV");
	stack3->GetYaxis()->CenterTitle();

	auto legend3 = new TLegend(0.6,0.5,0.85,0.85);
	for(unsigned int ihist = 0; ihist < idMatchPlots2->size(); ihist++){
		legend3->AddEntry(idMatchPlots2->at(ihist));
	}
	legend3->Draw();
	...
	/*
	TCanvas* pidEfficiencyCanvas = new TCanvas("pidEff", "pidEff", 1200,900);
	pidEfficiencyCanvas->Divide(2,2);
	vector<TH1F*>* splitPidGraphs[4] = {&lctPidEff, &lctPidEff4, &plctPidEff, &plctPidEff4};
	string pidTitles [4] = {"LCT Efficiency", "LCT Efficiency w/ >= 4 RH",
							"PLCT Efficiency", "PLCT Efficiency w/ >= 4 RH"};
	for(unsigned int i =0; i < 4; i++){
		pidEfficiencyCanvas->cd(i+1);
		vector<TH1F*>* thisVector = splitPidGraphs[i];
		THStack* thisStack = new THStack(string("pidEff"+to_string(i)).c_str(),
				pidTitles[i].c_str());


		for(unsigned int j = 0; j < thisVector->size()-1; j++){
			thisVector->at(j)->Divide(thisVector->back()); //divide by the last one, which ash everything
			thisStack->Add(thisVector->at(j));
		}

		thisStack->SetMinimum(0.5);
		thisStack->SetMaximum(1.1);
		thisStack->Draw("no stack");
		thisStack->GetXaxis()->SetTitle("Pt [GeV]");
		thisStack->GetXaxis()->CenterTitle();
		thisStack->GetYaxis()->SetTitle("Efficiency/0.5 GeV");
		thisStack->GetYaxis()->CenterTitle();
	...
	TCanvas* plctVsLctCanvas = new TCanvas("plctVsLctCanvas", "plctVsLctCanvas",1200,900);
	plctVsLctCanvas->cd();
	plctVsLctCanvas->SetLogy();
	THStack* plctVsLctBools = new THStack("plctVsLctBools", "PLCT vs LCT");
	lct1plct1->Divide(lctPlctDenom);
	lct1plct0->Divide(lctPlctDenom);
	lct0plct1->Divide(lctPlctDenom);
	lct0plct0->Divide(lctPlctDenom);


	plctVsLctBools->Add(lct0plct0);
	plctVsLctBools->Add(lct0plct1);
	plctVsLctBools->Add(lct1plct0);
	plctVsLctBools->Add(lct1plct1);
	plctVsLctBools->Draw();
	...
	TCanvas* setCompCanvas = new TCanvas("setComp", "setComp", 1200,900);
	//setCompCanvas->SetLogz();
	setCompCanvas->cd();
	for(int iy = 1; iy < patternOverlap->GetNbinsY()+1; iy++){
		double thisScale = patternOverlap->GetBinContent(iy,iy);
		for(int ix = 1; ix < patternOverlap->GetNbinsX()+1; ix++){
			double thisBinVal = patternOverlap->GetBinContent(ix,iy);
			patternOverlap->SetBinContent(ix, iy, thisBinVal/thisScale);
		}
	}

	//patternOverlap->Scale(1./patternOverlap->GetEntries());

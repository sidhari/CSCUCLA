    ...
    for(int pid = 10; pid >= 2; pid--)
    {
        ...
        for(int ST = 1; ST <= 4; ST++)
        {
            for(int RI = 1; RI <= 3; RI++)
            {
                ...
                TH1F *segA = (TH1F*)myF.Get(Form("SegA_me%i%i_pid%i_h",ST,RI,pid));
                mean[getCT(ST,RI)] = segA->GetMean();
                rms[getCT(ST,RI)] = segA->GetRMS();
        ...
        grPIDmean[pid]->SetTitle("Means of Angular Distributions");
        ...
        grPIDrms[pid]->SetTitle("RMSs of Angular Distributions");
        ...
        { 
            ...
            grPIDmean[pid]->GetYaxis()->SetTitle("Segment Angle Mean [strips/layer]");
            c1.cd();
            grPIDmean[pid]->Draw("AP"); fD = false; 
            grPIDrms[pid]->GetYaxis()->SetTitle("Segment Angle RMS [strips/layer]");
            c2.cd();

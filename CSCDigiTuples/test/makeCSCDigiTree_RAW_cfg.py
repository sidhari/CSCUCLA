...
#with open('fileListRECO_Char.txt') as freco:
#    recoFiles = freco.read().splitlines()

with open('fileListRAW.txt') as fraw:
    rawFiles = fraw.read().splitlines()

process.source = cms.Source ("PoolSource",
        fileNames = cms.untracked.vstring(rawFiles[fN])#,
...
process.MakeNtuple = cms.EDAnalyzer("CSCPatterns",
        NtupleFileName       = cms.untracked.string('root/Charmonium/CSCDigiTree' + str(options.firstFile) + '.root'),
...
process.p = cms.Path(process.RawToDigi * process.reconstruction * process.gtDigis * process.muonCSCDigis * process.csc2DRecHits * process.cscSegments * process.cscTriggerPrimitiveDigis * process.emtfStage2Digis * process.MakeNtuple)
